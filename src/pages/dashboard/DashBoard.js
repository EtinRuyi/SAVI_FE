...
const DashBoard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://localhost:7226/api/Saving/GetTotalSavingBalance?walletNumber=${localStorage.getItem("walletNumber")}`);
        const result = await response.json();
  ...
  return (
    ...
    <Row style={{ width: '100%' }}>
      ...
      {!isKycComplete && (
        ...
        <WelcomeBackJohnDoeParent style={{ marginBottom: '1em' }}>
          <WelcomeBackJohn1>
            Welcome back {capitalizeFirstLetter(localStorage.getItem("fullname"))}
          </WelcomeBackJohn1>
          <EmojiParent>
    ...
    <Row style={{ justifyItems: 'center',backgroundColor:' var(--grey-50)',width:'fit-content',paddingTop:'1.5em',paddingLeft:'1em',paddingRight:'1em'}}>
      ...
      >
        ...
        <GlobalWalletParent>
          <SafeLock style={{marginBottom:'1.5em'}}>{`Total Personal Savings `}</SafeLock>
